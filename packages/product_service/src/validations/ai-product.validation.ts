...
export const generateDescriptionSuggestionsSchema = Joi.object({
    ...
    count: Joi.number().integer().min(1).max(5).default(3).messages({
        ...
        "number.max": "Le nombre de suggestions ne peut pas dépasser 5",
    }),
});

export const improveDescriptionSchema = Joi.object({
    productName: Joi.string().min(2).max(100).trim().required().messages({
        "string.empty": "Le nom du produit est requis",
        "string.min": "Le nom du produit doit contenir au moins 2 caractères",
        "string.max": "Le nom du produit ne doit pas dépasser 100 caractères",
    ...
    currentDescription: Joi.string().min(10).max(1000).trim().required().messages({
        "string.empty": "La description actuelle est requise",
        "string.min": "La description doit contenir au moins 10 caractères",
        "string.max": "La description ne doit pas dépasser 1000 caractères",
        "any.required": "La description actuelle est requise",
    }),
    improvements: Joi.string().max(500).trim().optional().messages({
        "string.max": "Les améliorations demandées ne doivent pas dépasser 500 caractères",
        "string.base": "Les améliorations doivent être une chaîne de caractères",
    }),
});

export const generateKeywordsSchema = Joi.object({
    productName: Joi.string().min(2).max(100).trim().required().messages({
        "string.empty": "Le nom du produit est requis",
        "string.min": "Le nom du produit doit contenir au moins 2 caractères",
        "string.max": "Le nom du produit ne doit pas dépasser 100 caractères",
